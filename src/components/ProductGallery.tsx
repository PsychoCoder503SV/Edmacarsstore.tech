...
export function ProductGallery({ images, name }: Props) {
  ...
  return (
    <div>
      <div className="relative aspect-square overflow-hidden rounded-2xl border border-glass bg-zinc-950 glass-surface-elevated">
        <ProductImage src={list[active]} alt={name} fill fit="cover" priority />
      </div>
      {list.length > 1 && (
        <div className="mt-3 grid grid-cols-4 gap-2 sm:grid-cols-5">
          {list.map((url, index) => (
            ...
            >
              <ProductImage src={url} alt={`${name} ${index + 1}`} fill fit="cover" />
            </button>
          ))}
        </div>
      )}
